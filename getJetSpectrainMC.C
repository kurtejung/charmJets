...
void getJetSpectrainMC(bool doMatchingToData=0, int gsp=0){
  ...
  int nBins = 3; //not const because these can get modified to match to data
  float *ptBin = NULL;
  int ptBinDefaultBinning[4] = {40,80,150,400};
  ...
  TFile *fin = NULL;
  //if(gsp==0) fin = new TFile("input/DMesonCJet_QCDJetOnly_pPbMC_ppReco_akPu3PF_convertToJetTree.root");
  if(gsp==0) fin = new TFile("/Users/kjung/bTagTrees/pPb/histos/ppMC_ppReco_akPu3PF_QCDjetTrig_etashift_Fix2Sample_MCWeightFinalWithVz_noTrgSelection_Full.root");
  else if(gsp==2) fin = new TFile("/Users/kjung/bTagTrees/pPb/histos/pPbMC_ppReco_akPu3PF_QCDjetTrig_etashift_WeightCorr_useGSP2_noIPupperCut_7.root");
  else if (gsp==3) fin = new TFile("/Users/kjung/bTagTrees/pPb/histos/pPbMC_ppReco_akPu3PF_QCDjetTrig_etashift_WeightCorr_useGSP3_noIPupperCut_7.root");
  ...
  int cbinlo=-2, cbinhi=200;
    
  nt->Draw("jtpt>>hTagBJets",Form("weight*(%s>=%f && %s<=%f && abs(refparton_flavorForB)==4 && jteta>%f && jteta<%f &&subid==0)",discriminator.c_str(),discrCut,discriminator.c_str(),maxDiscrCut,etalo,etahi));
  nt->Draw("jtpt>>hTagJets",Form("weight*(%s>=%f && %s<=%f && jteta>%f && jteta<%f &&subid==0)",discriminator.c_str(),discrCut,discriminator.c_str(),maxDiscrCut,etalo,etahi));
  nt->Draw("jtpt>>hJets",Form("weight*(jteta>%f && jteta<%f && abs(refparton_flavorForB)<99&&subid==0)",etalo,etahi));
  nt->Draw("jtpt>>hBJets",Form("weight*((abs(refparton_flavorForB)==4) && jteta>%f && jteta<%f &&subid==0)",etalo,etahi));
  nt->Draw("jtpt>>hTaggedBJetsMC",Form("weight*(%s>=%f && %s<=%f&&bin>=%d&&bin<%d&&jteta>%f&&jteta<%f&&abs(refparton_flavorForB)==4&&subid==0)",discriminator.c_str(),discrCut,discriminator.c_str(),maxDiscrCut,cbinlo,cbinhi,etalo,etahi));
  nt->Draw("jtpt>>hUntaggedBJetsMC",Form("weight*((%s<%f || %s>%f)&&bin>=%d&&bin<%d&&jteta>%f&&jteta<%f&&abs(refparton_flavorForB)==4&&subid==0)",discriminator.c_str(),discrCut,discriminator.c_str(),maxDiscrCut,cbinlo,cbinhi,etalo,etahi));
  nt->Draw("jteta","weight*(abs(refparton_flavorForB)==4&&subid==0&&bin>=0&&bin<200)");
  ...
  TFile *fout = new TFile(Form("output/ppMC_akPu3PF_cJet_matchBin%d_gsp%d_JPLT%g_eta%.1fTo%.1f.root",doMatchingToData,gsp,discrCut,etalo,etahi),"recreate");
  fout->cd();
  
  cout << "before: " << hRawBMC->GetBinContent(1) << endl;
  ...
  cout << "after: "<< hRawBMC->GetBinContent(1) << endl;
