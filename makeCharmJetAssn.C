...
void makeCharmJetAssn(int isMC=1, bool doDraw = 0){
  ...
  for(int ientry=0; ientry<nentries; ientry++){
	...
	if(isMC){
	  ...
	  //check all partons to make sure *one of them* is in the jet cone
	...
	unmatchedCJets->Fill(jtpt->at(ijet));
      ...
      //do efficiency for dR matching
      if(minDR<999){
	cJetEffvsPt_bg_step1->Fill(jtpt->at(ijet));
	if(minDR<djetR_testCut){
	  cJetEffvsPt_step1->Fill(jtpt->at(ijet));
	}
	else{
	  cJetEffvsPt_rej_step1->Fill(jtpt->at(ijet));
	}
      }

      //do efficiency for D reco efficiency
      if(hasGenD->at(ijet)){
	cJetEffvsPt_bg_step2->Fill(jtpt->at(ijet));
	if(minDR<999){
	  cJetEffvsPt_step2->Fill(jtpt->at(ijet));
	}
	else{
	  cJetEffvsPt_rej_step2->Fill(jtpt->at(ijet));
	}
      }
      
      //do efficiency for D->hadron branching ratio [only makes sense for c-jet only sample]
      cJetEffvsPt_bg_step3->Fill(jtpt->at(ijet));
      if(hasGenD->at(ijet)==423) cJetEffvsPt_step3[0]->Fill(jtpt->at(ijet));
      else if(hasGenD->at(ijet)==421) cJetEffvsPt_step3[1]->Fill(jtpt->at(ijet));
      else if(hasGenD->at(ijet)==431) cJetEffvsPt_step3[2]->Fill(jtpt->at(ijet));
      //else if(hasGenD->at(ijet)==431) cJetEffvsPt_step3[3]->Fill(jtpt->at(ijet));
      else if(hasGenD->at(ijet)==411) cJetEffvsPt_step3[4]->Fill(jtpt->at(ijet));
      else cJetEffvsPt_rej_step3->Fill(jtpt->at(ijet));
