...
void drawRAA(bool doInclusiveJet=0, bool useUnfolded=0, bool drawTheory=0){
  ...
  //  if(icent==2){ binLo = 50; binHi=100; }
    ...
    else if(useUnfolded) f0 = new TFile(Form("~/bTagTrees/pPb/a%.1fTo%.1f.root",etalo,etahi)); //bjet unfolding
    else f0 = new TFile("/Users/kjung/charmJets/pPb/inputToRAA/raa_pPb_numerator-Cjet_MisalignOnlyDataset_ssvhe20_etaCM_v2_bin0_100_eta-2.0To2.0.root");
    ...
    else if(useUnfolded) f2 = new TFile(Form("~/bTagTrees/pPb/raa_pp_denomForpA-Bjet_etaCM_v2_bin0_100_eta%.1fTo%.1f.root",etalo,etahi),"OLD"); //incl jet unfolding
    else f2 = new TFile(Form("inputToRAA/raa_pp_denomForpA-Cjet_etaCM_v2_bin0_100_eta%.1fTo%.1f.root",etalo,etahi),"OLD"); //bjet unfolding
	...
	if((yerr[i-1]>0.001 && yp[i-1]>-1) || !useUnfolded){ 
	...
	pythiaSysErr[jj]*=(1+optionBExtra[jj]);
      ...
      pythiaErr = new TGraphErrors(1,pythiaXpoints,pythiaYpoints,pythiaXerr,pythiaSysErr);
    }
    ...
    hcln->SetXTitle("c-jet p_{T} [GeV/c]");
  ...
  leg1->AddEntry(systErr[0],"c-jet R_{pA}^{PYTHIA}, -2.4<#eta_{CM}<1.6","fp");
  //if(drawTheory) leg1->AddEntry(grepPbtheory,"Vitev, et.al. arXiv:1306.0909","lp");
  ...
  //if(drawTheory) grepPbtheory->Draw("C,same");
  ...
  cmsP->SetTextSize(25);
  cmsP->Draw("same");
  TLatex *l1 = new TLatex(-0.5,0.18,Form("0-100%%"));
  //l1->Draw("same");
  TLatex *l4 = new TLatex(160,2.56,"pPb L = 20.7 nb^{-1}");//; PbPb L = 150 #mub^{-1}");
  l4->SetTextFont(43);
  l4->SetTextSize(25);
  l4->Draw("same");
  TLatex *l2 = new TLatex(20.77,0.175,"pPb #sqrt{s_{NN}} = 5.02 TeV #int L dt = 20.7 nb^{-1}"); //35, 20.7, 14
  ...
  TLatex *ltag = new TLatex(65,2.2,"djetR < 0.3");
  //ltag->Draw("same");
  TFile *out = new TFile("RpA_CJet_Output.root","RECREATE");
  out->cd();
