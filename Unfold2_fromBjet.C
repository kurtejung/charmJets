...
#include "unfoldUtilities/utilities.h"
#include "unfoldUtilities/bayesianUnfold.h"
#include "unfoldUtilities/prior.h"
#include "unfoldUtilities/generateSmearingMatrix.h"
...
void Unfold2_fromBjet(int algo= 3,bool useSpectraFromFile=0, bool useMatrixFromFile=0, int doToy = 0, int isMC = 0,char *spectraFileName = (char*)"pbpb_spectra_ak3PF.root",double recoJetPtCut = 35., int nBayesianIter = 4, int doCjets=1, int doTrigEffCorr=0, int doSkew=0, int doJESshift=0, bool doResSmear=false, int centBin=0, double etalo=-2.0, double etahi=2.0, bool ispPb=1) 
{
  ...
  if(doCjets) fileNamePP_data = (char*)("output/pp2p76_NewFormatV11_ak3PF_fixBin_bFractionMCTemplate_pPbpp1_gsp0_jetptcut30__nofilter_vsSvtxm_SSVHPat1.7FixCL0_bin_0_100_eta_-2_2.root");
  else fileNamePP_data = (char*)"output/pp2p76_NewFormatV11_ak3PF_fixBin_bFractionMCTemplate_pPbpp1_gsp0_jetptcut30__nofilter_vsSvtxm_SSVHPat1.7FixCL0_bin_0_100_eta_-2_2.root";

  if(dopPbOnly){
    //if(doCjets) fileNamePbPb_data = (char*)"output/cJetFitterV3_looseDCuts_fitToJP_officialMC_akPu3PF_FirstHalf_consistentEta_ssvhe20_addlJetCuts_fixBin_bFractionMCTemplate_pPbpp1_gsp0_jetptcut30__nofilter_vsSvtxm_SSVHP_rebin3at1.7_FixCL0_bin_0_100_eta_-2.00_2.00.root";
    if(doCjets) fileNamePbPb_data = (char*)"output/cJetFitterV3_looseDCuts_fitToJP_officialMC_akPu3PF_FirstHalf_consistentEta_ssvhe20_addlJetCuts_fixBin_bFractionMCTemplate_pPbpp1_gsp0_jetptcut30__nofilter_vsSvtxm_SSVHPat1.7_FixCL0_bin_0_100_eta_-2.00_2.00.root";
    else fileNamePbPb_data = (char*)"histos/PUForest_akpu3pf_etaLT1_FirstHalf_noGplus_forUnfolding.root";
  }
  else{
    if(doCjets) fileNamePbPb_data = (char*)"output/cJetFitterV3_fitToJP_officialMC_akPu3PF_SecondHalfDataWithpPbMC_JPRetrain_v17JEC_consistentEta_ssvhe20_addlJetCuts_fixBin_bFractionMCTemplate_pPbpp1_gsp0_jetptcut30__nofilter_vsSvtxm_SSVHPat1.7FixCL0_bin_0_100_eta_-2.00_2.00.root";
    else fileNamePbPb_data = (char*)"histos/PUForest_akpu3pf_etaLT1_SecondHalf_noGplus_forUnfolding.root";
  }
 
  if(doCjets) fileNamePP_mc = (char*)"input/DMesonCJet_QCDJetOnly_ppMC_centReweight_ppReco_akPu3PF_gsp0_837.root";
  else fileNamePP_mc = (char*)"input/DMesonCJet_QCDJetOnly_ppMC_centReweight_ppReco_akPu3PF_gsp0_837.root";
  
  if(dopPbOnly){
    if(useOfficialMC) fileNamePbPb_mc = (char*)"input/DMesonCJet_QCDJetOnly_pPbMC_ppReco_akPu3PF_convertToJetTree_noCuts.root";
    else if(doCjets) fileNamePbPb_mc = (char*)"histos/pPbMC_ppReco_akPu3PF_QCDjetTrig_etashift_noTrgSelection_MCWeightFinalWithSmoothedVzCentWeight_VzCut_FixWeight_8.root";
  ...
  if(doPbpOnly){
    if(useOfficialMC)fileNamePbPb_mc = (char*) "input/DMesonCJet_QCDJetOnly_pPbMC_ppReco_akPu3PF_convertToJetTree_noCuts.root";
    else if(doCjets)fileNamePbPb_mc = (char*) "histos/PbpMC_ppReco_akPu3PF_QCDjetTrig_etashift_noTrgSelection_v17JECcheck_MCWeight_NoVzCentWeight_10.root";
  ...
  if (isMC) {
    ...
    if(dopPbOnly) pbpb_Unfo  = new TFile(Form("pPb_Unfo_inCM_v31_officialMC_%s_%s_noGplus_testRebin_FirstHalfOnly_Converged_usedParameterizedUnfold%d_jtpt%.0f_%s_clo%d_chi%d_v8%s%s%s_eta_%.2fTo%.2f_.root",jetType.c_str(),algoName[algo],doParameterizedMatrix,recoJetPtCut,bJetString.c_str(),cBinLo,cBinHi,skewString.c_str(),resSmearString.c_str(),shiftString.c_str(),etalo,etahi),"RECREATE");
  ...
  JetDatapPb *dataPbPb   = new JetDatapPb(fileNamePbPb_mc,(char*)"jets"); // pPb data	
  JetDataPP *dataPP = new JetDataPP(fileNamePP_mc,(char*)"jets");	// pp data
  ...
  if (!useMatrixFromFile) {
    for (Long64_t jentry2=0; jentry2<dataPbPb->tJet->GetEntries();jentry2++) {
      ...
      if (doCjets && fabs(dataPbPb->refparton_flavorForB)!=4) continue;
      ...
      //if (doCjets&& dataPbPb->discr_ssvHighEff<2) continue;
     ...
     if(doJESshift==1) jtpt*=(1.-sqrt(8.)*0.01);
      ...
      //if(!doCjets)if(dataPbPb->refpt < 50 && dataPbPb->jtptA>120) continue;
      //if(doCjets)if(dataPbPb->refpt < 50 && dataPbPb->jtptB>120) continue;
     ...
     if(doTrigEffCorr){
       for(int iBin = 0; iBin<nbins_rec; iBin++){
         if(jtpt > boundaries_rec[iBin] && jtpt < boundaries_rec[iBin+1]){
           if(doCjets) weight/= trigEffbJet[iBin];
    ...
    for (Long64_t jentry2=0; jentry2<dataPP->tJet->GetEntries();jentry2++) {
      ...
      if ( doCjets && fabs(dataPP->refparton_flavorForB)!=4) continue; //need to fill the b-jets only for the pp Gen MC - its my denominator!
      //if ( doCjets && dataPP->discr_ssvHighEff<2) continue;
     ...
     if(doTrigEffCorr){
       for(int iBin = 0; iBin<nbins_rec; iBin++){
         if(jtpt > boundaries_rec[iBin] && jtpt < boundaries_rec[iBin+1]){
           if(doCjets) weight/= trigEffppbJet[iBin];
  ...
  if (isMC==0) {
    ...
    TFile *backup;
    if(dopPbOnly) backup = new TFile("input/DMesonCJet_pPbData_ppReco_akPu3PF_convertToJetTree_withLHCbVars_noCuts.root");
    else backup = new TFile("input/DMesonCJet_pPbData_ppReco_akPu3PF_convertToJetTree_withLHCbVars_noCuts.root");
    if(!doCjets){
      cout << "warning - redrawing from ntuple!! Only do this for inclusive-jet!" << endl;
      TTree *nt = (TTree*)backup->Get("jets");
	...
	if(doTrigEffCorr){
	  float trigEff = 0;
	  if(doCjets) trigEff = trigEffbJet[ibin-1];
  ...
  TH1F *hTagEffPP = NULL;
  if(doCjets){
      hMattPP = (TH1F*) MCtruth->Get("hRawBData"); //MODIFIED BECAUSE pA pp reference is MC!
    ...
    //else hMattPP = (TH1F*) infMattPP->Get("hIncJetsData");
      ...
      TFile *ppJet = new TFile("input/DMesonCJet_2p76Data_NoJetTrgCut_ppdata_ppReco_convertToJetTree.root");
      TTree *ct = (TTree*)ppJet->Get("jets");
      hMattPP = new TH1F("hMattPP","",nbins_rec,boundaries_rec); hMattPP->Sumw2();
      hTagEffPP = new TH1F("hTagEffPP","",nbins_rec,boundaries_rec);
      ct->Draw("jtpt>>hMattPP","weight*(abs(jteta)<2 && abs(vz)<15 && rawpt>25)");
      for(int i=1; i<=nbins_rec; i++){
        hTagEffPP->SetBinContent(i,1.);
        hTagEffPP->SetBinError(i,0.0001);
      }
    ...
    for (int i=1;i<=hMattPP->GetNbinsX();i++)
    {
     ...
     if(doCjets){
       ...
       if(tagEff>0){
	       // careful of the order here!
	       binError /= tagEff;// *sqrt(tagEffErr*tagEffErr/tagEff/tagEff + binError*binError/binContent/binContent);
	       binContent /= tagEff;
       }
  ...
  for (int i=0;i<=nbins_cent;i++) {
    ...
    TH2D *invertedMatrix = (TH2D*)uhist[i]->hMatrix->Clone("invertedMatrix");
    for(int ixbin=1; ixbin<=invertedMatrix->GetNbinsX(); ixbin++){
      for(int iybin=1; iybin<=invertedMatrix->GetNbinsY(); iybin++){
        invertedMatrix->SetBinContent(ixbin,iybin,uhist[i]->hMatrix->GetBinContent(iybin,ixbin));
        invertedMatrix->SetBinError(ixbin,iybin,uhist[i]->hMatrix->GetBinError(iybin,ixbin));
      }
    }

    //Lets use the real RooUnfold...
    RooUnfoldResponse ruResponse(uhist[i]->hMatrix->ProjectionY(), uhist[i]->hMatrix->ProjectionX(), invertedMatrix,"","");
	...
	//switching to SVD systematics
     RooUnfoldBayes unfoldSVDsys(&ruResponse, uhist[i]->hMeas, nBayesianIter+j);
