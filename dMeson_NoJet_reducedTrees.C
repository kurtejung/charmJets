...
const double tightSignalCutHi[cTypes] = {0.147, 1.875, 1.99, 1.99, 1.885};
...
void dMeson_NoJet_reducedTrees(){

  //TFile *fin = new TFile("input/DMesonCJet_pPbData_ppReco_akPu3PF_convertToJetTree_withLHCbVars_looseDCuts.root");
  //TTree *mt = (TTree*)fin->Get("dMesons");

  TFile *finMC = new TFile("input/DMesonCJet_QCDJetOnly_pPbMC_ppReco_akPu3PF_convertToJetTree_looseDCuts.root");
  TTree *mt2 = (TTree*)finMC->Get("dMesons");

  if(!mt2){ cout << "file or tree not found!!" << endl; exit(0); }
  double dCandPt, dCandMass, dCandEta, dCandPhi, dCandChildMass, dCandType, dCandCharge1, dCandCharge2;
  double ffls3d, falpha0, fprob;
  double weight;
  ...
  for(int i=0; i<8; i++){
    mass[i] = new TH1D(Form("mass_pt%i",i),"",20,1.75,1.95); mass[i]->Sumw2();
    massMC[i] = new TH1D(Form("massMC_pt%i",i),"",20,1.75,1.95); massMC[i]->Sumw2();
  }
  ...
  for(int ientry=0; ientry<nentries; ientry++){
    mt->GetEntry(ientry);
      ...
      if(dCandCharge1!=dCandCharge2 && dCandType==2){ //D0's only for right now
        if(ffls3d>3.5 && falpha0<0.06 && fprob>0.25){
         ...
         if(pt>150 || jj>8) jj = 8;
	   //cout << "pt " << pt << endl;
	   //cout << "filling ptbin: "<< jj << " with mass "<< dCandMass << " weight " << weight << endl;
         mass[jj-1]->Fill(dCandMass,weight);
     }
   }
 }*/

 cout << "nentries mc: "<< mt2->GetEntries() << endl;
 int nentries = mt2->GetEntries();
 for(int ientry=0; ientry<nentries; ientry++){
  mt2->GetEntry(ientry);
      if(dCandCharge1!=dCandCharge2 && dCandType==2){ //D0's only for right now
        if(ffls3d>3.5 && falpha0<0.06 && fprob>0.25){
     ...
     //cout << "pt " << pt << endl;
     //cout << "filling ptbin: "<< jj << " with mass "<< dCandMass << " weight " << weight << endl;
         massMC[jj-1]->Fill(dCandMass,weight);
       }
     }
 }
